...
def singasong():
    ...
    while Complite == False:
        ...
        World.slow_print(style.RED+"Lay down your pieces"+style.RESET,0.01 )
        World.layDown()
        World.slow_print(style.RED+"And let's begin"+style.RESET,0.01 )
        World.slow_print(style.GREEN+" OBJECT_CREATION"+style.RESET,0.01 )
                ...
                if data["Circel"] == len(data["setOfPoint"]):
                    ...
                    if len(data["sineWave"]) == 11:
                        ...
                        if data["im"] == "INFINITY":
                            ...
                            time.sleep(1.15)
                            World.slow_print(style.GREEN+"\nSwitch my current"+style.RESET,0.02 )
                            ...
                            World.slow_print(style.GREEN+"the trance"+style.RESET,0.04 )
                            time.sleep(1.5)
                            World.EnterTrance("worldExecuteMe.py") #!DEBUG CHECK
                            print(style.MAGENTA+"\nIF I CAN")
                            time.sleep(1)
                            World.slow_print(style.MAGENTA+"IF I CAN "+style.RESET,0)
    ...
    time.sleep(1.5) #! DEBUG TIME DELAY
...
def play_audio_async( device_index=None):
    
    internal_dir = sys._MEIPASS
    
    # Construct the path to the embedded WAV file
    embedded_wav_path = os.path.join(internal_dir, "worldexecuteme.wav")
    
    try:
        # Open the audio file
        wf = wave.open(embedded_wav_path, 'rb')

        # Create an instance of PyAudio
        p = pyaudio.PyAudio()

        # Get the device info for the specified device index (or use default)
        ...
        # Read and play the audio data in chunks asynchronously
        chunk_size = 1024
...
def main():
    thread1 = threading.Thread(target=singasong)
    thread2 = threading.Thread(target=play_audio_async)
    thread1.start()
    thread2.start()
    start_event.set()
    thread1.join()
...
if __name__ == "__main__":
    os.system("title World.Execute(Me)")
    main()
